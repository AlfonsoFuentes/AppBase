        ...
        protected override async Task OnInitializedAsync()
        {
            ...
            HubConnection.On<string>(ApplicationConstants.SignalR.PingResponse, (userId) =>
            {
                ...
                if (connectedUser is { IsOnline: false })
                {
                    ...
                    //_snackBar.Add($"{connectedUser.UserName} {"Logged In."}", Severity.Info);
                    StateHasChanged();
                }
            });
            ...
            HubConnection.On<string>(ApplicationConstants.SignalR.DisconnectUser, (userId) =>
            {
                ...
                if (disconnectedUser is { IsOnline: true })
                {
                    ...
                    _snackBar.Add($"{disconnectedUser.UserName} {"Logged Out."}", Severity.Info);
                    StateHasChanged();
                }
            });
            HubConnection.On<ChatHistory<IChatUser>, string>(ApplicationConstants.SignalR.ReceiveMessage, async (chatHistory, userName) =>
             {
                 if (CId == chatHistory.ToUserId && CurrentUserId == chatHistory.FromUserId || CId == chatHistory.FromUserId && CurrentUserId == chatHistory.ToUserId)
                 {
                     ...
                     {
                         _messages.Add(new ChatHistoryResponse { Message = chatHistory.Message, FromUserFullName = userName, CreatedOn = chatHistory.CreatedOn, FromUserImageURL = CurrentUserImageURL });
                         await HubConnection.SendAsync(ApplicationConstants.SignalR.SendChatNotification, string.Format("New Message From {0}", userName), CId, CurrentUserId);
                     }
                     else if (CId == chatHistory.FromUserId && CurrentUserId == chatHistory.ToUserId)
                     {
                         _messages.Add(new ChatHistoryResponse { Message = chatHistory.Message, FromUserFullName = userName, CreatedOn = chatHistory.CreatedOn, FromUserImageURL = CImageURL });
                     }
                     await _jsRuntime.InvokeAsync<string>("ScrollToBottom", "chatContainer");
                     StateHasChanged();
                 }
